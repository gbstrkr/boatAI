...
for i, odds_row in odds_data.iterrows():
    ...
    if not first_place_row.empty:
        ...
        if first_place_row.iloc[0]['予測']>0.1:
            # 単勝番号と予測結果1の艇番が一致するか確認
            if odds_row['複勝番号1'] == predicted_boat_number:
                # 一致すれば単勝オッズを加算
                final_value += odds_row['複勝オッズ1']
                final_value -= 100
                cnt_true+=1
            elif odds_row['複勝番号2'] == predicted_boat_number:
                final_value += odds_row['複勝オッズ2']
                final_value -= 100
                cnt_true+=1
            else:
                # 一致しなければ-100を減算
